...
- (void)deletePhoto:(UIButton *)sender{
    ...
    [self.LQPhotoPicker_delegate ButtonTag:sender.tag];
...
- (void)getSelectImgWithALAssetArray:(NSArray*)ALAssetArray thumbnailImgImageArray:(NSArray*)thumbnailImgArray{
    ...
    if (_LQPhotoPicker_delegate && [_LQPhotoPicker_delegate respondsToSelector:@selector(SaveResqustUpPhotoClick)]) {
//      [_LQPhotoPicker_delegate SaveResqustUpPhotoClick];
    }
...
#pragma mark ===================ZL添加拍照上传==================
- (void)getPaiZhaoImageURLwith:(NSString *)imageURL{
...
- (NSMutableArray*)LQPhotoPicker_getBigImageArray{
    _LQPhotoPicker_bigImageArray = [NSMutableArray array];
        ...
        for (UIImage *set in _LQPhotoPicker_selectedAssetArray) {
            [_LQPhotoPicker_bigImageArray addObject:set];
        }
