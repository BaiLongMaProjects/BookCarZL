...
@property (weak, nonatomic) IBOutlet UITextField *TextFieldMyArea;//填写区域

@property (weak, nonatomic) IBOutlet UIButton *ButMyArea;//保存按钮
@end
...
- (void)viewDidLoad {
    [super viewDidLoad];
       self.TextFieldMyArea.text = self.myArea;
    self.view.backgroundColor = BaseLDXColor;
    // Do any additional setup after loading the view from its nib.
}
//保存按钮
- (IBAction)ButSaveClick:(id)sender {
    if ([self.TextFieldMyArea.text isEqualToString:@""]) {
        [[RYHUDManager sharedManager] showWithMessage:@"您输入的所属区域不能为空" customView:nil hideDelay:2.f];
        return;
    }
    InforModel * infor = [[InforModel alloc]init];
    infor = [LoginDataModel sharedManager].inforModel;
    infor.company1 = self.TextFieldMyArea.text;
